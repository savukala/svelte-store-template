...
//  export let totalPriceFormula = products.products[0].price * circle_count + products.products[1].price * rectangle_count + products.products[2].price * triangle_count;
  ...
  fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur&order=market_cap_desc&per_page=1000&page=1&sparkline=false')
  .then(
    function(response) {
...
<div class="coin-value-wrapper">
  <p>Bitcoin has current value of: {coinValue} euro</p>
  <p>Euro has a current value of: {inverseCoinValue} btc</p>
